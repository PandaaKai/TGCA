...
from .kan_layer_fourier import NaiveFourierKANLayer as fftKanLayer
from .kan import KANLinear as effKanLayer

class KanGNN(nn.Module):
    def __init__(self, in_size, hidden_size, out_size, heads, num_layers, dropout=0.):
        ...
        self.lin = nn.Linear(self.in_size, self.hidden_size)
        ...
        for i in range(self.num_layers // 2):
            ...
            self.gnn_layers.append(
                effKanLayer(hidden_size, hidden_size)
            )
        self.gnn_layers.append(
    ...
    def forward(self, x, adj):
        ...
        #     adj, x = self.gnn_layers[i]((adj, x))
        # for i in range(self.num_layers):
        #     x = self.gnn_layers[self.num_layers+i](x)

        for i in range(self.num_layers // 2):
            adj, x = self.gnn_layers[2*i]((adj, x))
            x = self.gnn_layers[2*i+1](x)
